...
class b07_samplelog_parser:
    def __init__(self, dirpath, infile, outfile):
        ...
        if "entry" in self.filedata.keys():
            self.entry_path = "entry"
        elif "entry1" in self.filedata.keys():
            self.entry_path = "entry1"
        else:
            raise ValueError("Path for entry item in nexus file could not be found")
        ...
        self.standard_columns = {
            ...
            "cff": f"{self.entry_path}/instrument/pgm_cff/value",
            "Endstation": self.get_endstation,
            "s4b_Ygapsize": f"{self.entry_path}/instrument/s4b_ygapsize/value",
            "Scan Command": f"{self.entry_path}/scan_command",
    ...
    def get_sm21b_x(self):
        if (self.get_endstation() == 2) & (
            "sm21b_x" in self.filedata[f"{self.entry_path}/instrument"].keys()
        ):
            return self.filedata[f"{self.entry_path}/instrument/sm21b_x/value"]
        return " "

    def get_sm21b_y(self):
        if (self.get_endstation() == 2) & (
            "sm21b_y" in self.filedata[f"{self.entry_path}/instrument"].keys()
        ):
            return self.filedata[f"{self.entry_path}/instrument/sm21b_y/value"]
        return " "

    def get_sm21b_z(self):
        if (self.get_endstation() == 2) & (
            "sm21b_z" in self.filedata[f"{self.entry_path}/instrument"].keys()
        ):
            return self.filedata[f"{self.entry_path}/instrument/sm21b_z/value"]
        return " "

    def get_sm21b_roty(self):
        if (self.get_endstation() == 2) & (
            "sm21b_roty" in self.filedata[f"{self.entry_path}/instrument"].keys()
        ):
            return self.filedata[f"{self.entry_path}/instrument/sm21b_roty/value"]
        return " "

    def get_sm52b_xp(self):
        if self.get_endstation() == 1:
            return self.filedata[f"{self.entry_path}/instrument/sm52b_xp/value"]
        return " "

    def get_sm52b_yp(self):
        if self.get_endstation() == 1:
            return self.filedata[f"{self.entry_path}/instrument/sm52b_yp/value"]
        return " "

    def get_sm52b_zp(self):
        if self.get_endstation() == 1:
            return self.filedata[f"{self.entry_path}/instrument/sm52b_zp/value"]
        return " "

    def get_sm52b_rotY(self):
        if self.get_endstation() == 1:
            return self.filedata[f"{self.entry_path}/instrument/sm52b_roty/value"]
        return " "

    def get_sm52b_rotZ(self):
        if self.get_endstation() == 1:
            return self.filedata[f"{self.entry_path}/instrument/sm52b_rotz/value"]
        return " "
    ...
    def get_photon_energy(self):
        if self.get_scan_type() == "XPS":
            return str(
                self.filedata[f"{self.entry_path}/instrument/pgm_energy/value"]
            ).replace("\n", "")
        return " "
    ...
    def get_date_str(self):
        ...
        dt_object = datetime.fromisoformat(
            self.filedata[f"{self.entry_path}"]
            .start_time.nxdata.decode()
            .replace("Z", "")
            .split("+")[0]
        )
    ...
    def get_nexafs_regions(self):
        ...
        #     return str(
        #         self.filedata[f"{self.entry_path}/analyser/region_list"].astype(str)
        #     )
        return ""
